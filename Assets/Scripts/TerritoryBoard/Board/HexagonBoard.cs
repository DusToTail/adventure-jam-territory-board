...
namespace TerritoryBoard
{
    public class HexagonBoard : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            Vector3 boardSpawnPos = boardConfig.spawnPosition;
            ...
            _grid.ForEach(cell =>
            {
                ...
                float _x = boardSpawnPos.x + cell.X * HexagonTileUtilities.WidthInterval * hexagonSize + (isForwardOffsetHeight ? HexagonTileUtilities.EdgeDistance * hexagonSize : 0);
                float _y = boardSpawnPos.y;
                float _z = boardSpawnPos.z + cell.Y * 1.5f * hexagonSize;
                Vector3 tilePos = new Vector3(_x,_y,_z);

                var go = Instantiate(prefab, tilePos, Quaternion.identity, transform);
                var hexTile = go.GetComponent<HexagonTile>();
                hexTile.SetCell(cell);
                hexTile.SetDefaultWorldPosition(tilePos);
